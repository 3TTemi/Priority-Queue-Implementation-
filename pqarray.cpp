...
/*
Enqueue function adds Datapoint to the end of the list and readjusts to the correct position
1. Chekcs if array is full, and calls enlarge function if so
2. Sets new element to paramter
3.Increments num filled variable
4.For every index in queue, swap if previous priority is less than current priority, if not stop
...
1.Creates a new array double the allocation of the previous
2.Copies all elemtns from previous array usign for loop
3. Deletes previous array
4.Sets array to newly created array (changing pointers)
5.Adjusts numAllocated vlaue by 2 for private refernce
 */

void PQArray::enlargeArray()
{
    DataPoint* newArray = new DataPoint[_numAllocated * 2];
...
TEST("PQArray enqueuing element last with lowest priority")
{
    PQArray pq;
...
TEST("PQArray checking the size of an enqueued fucntion")
{
    PQArray pq;
...
TEST("PQArray timing operation test of dequeueing") {
    PQArray pq;
    int n = 200000;

    fillQueue(pq, n);
    TIME_OPERATION(150000, emptyQueue(pq, 100000));
    TIME_OPERATION(150000, emptyQueue(pq, 50000));
    TIME_OPERATION(150000, emptyQueue(pq, 25000));
    TIME_OPERATION(150000, emptyQueue(pq, 10000));

}

TEST("PQArray timing operation test of enqueueing") {
    PQArray pq;
    int n = 0;
...
TEST("PQArray example from writeup") {
    PQArray pq;
...
_test_TEST("PQArray, enqueue only, validate at every step") {
    PQArray pq;
...
TEST("PQArray: operations size/isEmpty/clear") {
    PQArray pq;
...
TEST("PQArray: dequeue or peek on empty queue raises error") {
    PQArray pq;
...
TEST("PQArray, dequeue, validate at every step") {
    PQArray pq;
...
TEST("PQArray, test enlarge array memory") {
...
TEST("PQArray, sequence of mixed operations") {
    PQArray pq;
    int size = 30;
    double val = 0;
...
TEST("PQArray stress test, cycle many random elements in and out") {
    PQArray pq;
...
TEST("PQArray timing test, fillQueue and emptyQueue") {
    PQArray pq;
    int n = 20000;
